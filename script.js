...
const actualizarDatos = () => {
  fetch(bitcoinPriceUrl, {
    ...
    .then((data) => {
      ...
      if (moneda.value == "USD"){
        const dolar = data.bpi.USD;
        priceElement.innerHTML = `${dolar.code}${dolar.symbol} ${dolar.rate}`;
      } else if (moneda.value == "GBP"){
        const libra = data.bpi.GBP;
        priceElement.innerHTML = `${libra.code}${libra.symbol} ${libra.rate}`;
      } else if (moneda.value == "EUR"){
        const euro = data.bpi.EUR;
        priceElement.innerHTML = `${euro.code}${euro.symbol} ${euro.rate}`;
      }
